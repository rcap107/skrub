"""
Adapting skrub: from a traditional pipeline to DataOps
==================================================================

In this example we show how we can go from a traditional pipeline that uses
Pandas, scikit-learn and skrub, to skrub Data Ops, with minimal changes.
...
This is a shortened version of the :ref:`sphx_glr_auto_examples_data_ops_1120_multiple_tables.py`
example: we focus on the code for building the pipelines. Please refer to the
full example for more details and explanations about the dataset, the problem, and the
results.
...
baskets = pd.read_csv(dataset.baskets_path)
products = pd.read_csv(dataset.products_path)

kept_products = products.merge(
    baskets[["ID"]], left_on="basket_ID", right_on="ID", how="inner"
)
kept_products["total_price"] = (
    kept_products["Nbr_of_prod_purchas"] * kept_products["cash_price"]
)
